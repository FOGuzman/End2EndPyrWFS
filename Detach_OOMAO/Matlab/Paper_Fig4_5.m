...
savePath = "./ComputeResults/paperRR/Fig4/";if ~exist(savePath, 'dir'), mkdir(savePath); end
matName = "r0PerformanceFig4";
...
% Phisycal parameters
run("./tools/experiments_settings/F_5_6_settings.m")
...
y1 = R{1}.RMSEpyr(1,:);
s1 = R{1}.RMSEpyr(2,:);
s1 = squeeze(std(R{1}.Alldata(1,:,:),[],2))';

y2 = R{2}.RMSEpyr(1,:);
s2 = R{2}.RMSEpyr(2,:);
s2 = squeeze(std(R{2}.Alldata(1,:,:),[],2))';

y3 = R{3}.RMSEpyr(1,:);
s3 = R{3}.RMSEpyr(2,:);
s3 = squeeze(std(R{3}.Alldata(1,:,:),[],2))';

y4 = R{4}.RMSEpyr(1,:);
s4 = R{4}.RMSEpyr(2,:);
s4 = squeeze(std(R{4}.Alldata(1,:,:),[],2))';

y5 = R{1}.RMSEdpwfs(1,:);
s5 = R{1}.RMSEpyr(2,:);
s5 = squeeze(std(R{1}.Alldata(2,:,:),[],2))';
...
errorbar(r0s,y1,s1,'--','LineWidth',lwd,'Color',[250 0 0]/255)
hold on
errorbar(r0s,y2,s2,'--','LineWidth',lwd,'Color',[255 100 0]/255)
errorbar(r0s,y3,s3,'-.','LineWidth',lwd,'Color',[235 220 0]/255)
errorbar(r0s,y4,s4,':','LineWidth',lwd,'Color','m')
errorbar(r0s,y5,s5,'-b','LineWidth',lwd)
