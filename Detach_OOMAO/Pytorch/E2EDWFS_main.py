...
from modelDWFS import WFSModel,PhaseConstraint
...
model_path = "./modelDWFS/freedata_withconv/" + sub_fold + "/checkpoint"
result_path = "./freedata_results"
log_path   = "./modelDWFS/freedata_withconv/" + sub_fold + "/"
load_train = 0
nEpochs    = 120
lr         = 0.002
...
if load_train != 0:
    PyrNet = torch.load(model_path + "/DENet_epoch_{}.pth".format(load_train))
...
def test(test_path, epoch, result_path, model):
    ...
    for i in range(len(test_list)):
        ...
        with torch.no_grad():
            ...
            if Ypyr_res is not None:
                Ypyr_res = torch.concat([Ypyr_res,Ypyr.cpu()],0)
    ...
    print(prtname)
    OL1_trained = torch.fft.fftshift(PyrNet.state_dict()['prop.OL1'].cpu())
