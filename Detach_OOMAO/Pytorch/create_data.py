...
for it in range(NData):
 ...
 r0            = random.uniform(Rrange[0],Rrange[1])
 ...
 scio.savemat(TrainFolder + "data_{}.mat".format(np.int(it)) 
                 , {'x': np.squeeze(phaseMap.numpy()),'Zgt':Zgt.numpy(),'Zpyr':Zpyr.numpy()})
 
 if it%100 == 0:
    print('Creating data ({}-{})'.format(np.int(it),np.int(NData)))  
