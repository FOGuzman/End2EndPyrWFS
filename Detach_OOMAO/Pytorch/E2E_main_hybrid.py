...
parser.add_argument('--model', default="modelFastplusConvNeXt", type=str)
...
def train(epoch, model, lr,paths):
    epoch_loss = 0
    begin = time.time()
    optimizer_de = optim.AdamW([{'params': model.prop.OL1}], lr=lr*10)
    optimizer_g = optim.AdamW([{'params': model.prop.NN.parameters()}], lr=lr)

    if (epoch % 2 == 0):
        model.prop.OL1.requires_grad = False
    else:
        model.prop.OL1.requires_grad = True
